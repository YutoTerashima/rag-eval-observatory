...
Purpose: Retrieval-miss taxonomy and failure viewer serialization.
...
MODULE_PURPOSE = 'Retrieval-miss taxonomy and failure viewer serialization.'
...
def classify_retrieval_failure(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def failure_viewer_record(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = classify_retrieval_failure(materialized)
...
def difficulty_breakdown(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
