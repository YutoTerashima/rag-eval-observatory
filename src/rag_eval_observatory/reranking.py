...
"""Project-specific research helpers for RAG Eval Observatory.

Purpose: Rerank-depth and score-normalization utilities for retrieval ablations.
...
MODULE_PURPOSE = 'Rerank-depth and score-normalization utilities for retrieval ablations.'
...
def rerank_candidates(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def normalize_scores(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = rerank_candidates(materialized)
...
def rerank_depth_curve(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
