...
Purpose: Lexical, dense-projection, and hybrid retrieval helpers.
...
MODULE_PURPOSE = 'Lexical, dense-projection, and hybrid retrieval helpers.'
...
def rank_contexts(rows: Iterable[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def hybrid_score(rows: Iterable[Mapping[str, Any]], label: str = "experiment_id") -> dict[str, float]:
    """Summarize the project-specific tradeoff metrics used in the report."""
    materialized = [dict(row) for row in rows]
    if not materialized:
        return {"rows": 0.0, "best_quality": 0.0, "mean_quality": 0.0}
    ranked = rank_contexts(materialized)
...
def recall_at_k(records: Iterable[Mapping[str, Any]], field: str = "failure_type") -> list[dict[str, Any]]:
